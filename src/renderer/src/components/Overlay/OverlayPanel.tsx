...
export function OverlayPanel(): React.JSX.Element {
  ...
  const handleEndDay = (): void => {
    endDay(todayRef.current)
  }
  const handleContinueDay = (): void => {
    continueDay(todayRef.current)
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0">
        <AnchorWidget
          onOpenSettings={() => setView(view === 'settings' ? 'timeline' : 'settings')}
        />
      </div>
      ...
      >
        <span className="text-sm font-medium text-text-primary flex-1">
          ...
          {dayBoundary ? (
            <span className="text-text-muted font-normal">
              · started{' '}
              {new Date(dayBoundary.startTime).toLocaleTimeString([], {
                hour: '2-digit',
                minute: '2-digit'
              })}
            </span>
          ...
          {dayBoundary?.endTime ? (
            <span className="text-text-muted font-normal">
              {' '}
              →{' '}
              {new Date(dayBoundary.endTime).toLocaleTimeString([], {
                hour: '2-digit',
                minute: '2-digit'
              })}
            </span>
        ...
        {!dayBoundary ? (
          <button
            ...
            className="text-sm px-4 py-1.5 rounded-lg bg-accent text-[#0a0c10] font-semibold hover:bg-accent-hover transition-colors shadow-[0_0_14px_rgba(14,165,233,0.22)]"
          >
            Start Day
          </button>
        ) : !dayBoundary.endTime ? (
          <button
            ...
            className="text-sm px-4 py-1.5 rounded-lg border border-border text-text-muted hover:text-text-primary hover:border-border-hover transition-colors bg-surface"
          >
            End Day
          </button>
        ) : (
      ...
      <div className="flex-1 min-h-0 flex flex-row">
        <SideNav currentView={view} onViewChange={setView} />
